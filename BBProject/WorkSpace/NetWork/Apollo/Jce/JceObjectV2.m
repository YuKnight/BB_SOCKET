...
- (NSString *)description
{
    return [NSString stringWithFormat:
            ...
            @"    type     : %@\n"
            @"    ext      : %@\n"
            @"}\n", (long)self.tag, self.flag, self.name, self.type, self.ext];
}
        ...
        if ((propsDescription = [jcev2_class_props_descriptions objectForKey:className]) == nil) 
        {
            ...
            for (NSString *propName in props) 
            {
                if ([propName hasPrefix:JV2_PROP_LFX_STR]) {
                    ...
                    [propsDescription setObject:propInfo forKey:[NSNumber numberWithInt:(int)propInfo.tag]];
                }
            }
        }
        ...
        switch ([propInfo.type characterAtIndex:0]) 
        {
            ...
            case 'q':   // long long
                if (propInfo.flag != NO || theProperty != 0)
                    [stream writeInt:[theProperty longLongValue] tag:(int)propInfo.tag];
                break;
            case 'f':   // float
                if (propInfo.flag != NO || theProperty != 0)
                    [stream writeFloat:[theProperty floatValue] tag:(int)propInfo.tag];
                break;
            case 'd':   // double
                if (propInfo.flag != NO || theProperty != 0)
                    [stream writeDouble:[theProperty doubleValue] tag:(int)propInfo.tag];
                break;
            case '@':   // objects
                if (propInfo.flag != NO || theProperty != nil)
                    [stream writeAnything:theProperty tag:(int)propInfo.tag required:propInfo.flag];
                break;
            default:
                assert(0);
                break;
...
- (void)__unpack:(JceInputStream *)stream
{
    ...
    for (NSNumber* key in keys) 
    {
        ...
        switch ([propInfo.type characterAtIndex:0]) 
        {
            ...
            case 'd':   // double
            {
                NSNumber *value = [stream readNumber:(int)propInfo.tag required:propInfo.flag];
                if (value != nil)
            ...
            case '@':   // objects
            {
                ...
                id value = [stream readAnything:(int)propInfo.tag required:propInfo.flag description:(propInfo.ext != nil ? propInfo.ext : cls)];
                if (value != nil) {
